...
const AdminRoomAnalytics = () => {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/room-requests/admin/analytics?${queryParams}`, {
        headers: {
      ...
      if (response.ok && result.success) {
        ...
        showNotification(result.message || 'Failed to fetch room allocation analytics', 'error');
      }
    } catch (error) {
      console.error('Error fetching analytics:', error);
      showNotification('Failed to fetch room allocation analytics', 'error');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const filteredRequests = approvedRequests.filter(request => {
    const matchesSearch = request.preferred_room_type?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         ...
                         request.room_allocations?.[0]?.rooms?.room_number?.toString().includes(searchTerm);
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-slate-600">Loading room allocation analytics...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-100">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-slate-800">Room Allocation Analytics & Performance</h2>
              <p className="text-slate-600 mt-1">View all approved room allocations and performance metrics</p>
              <div className="flex items-center space-x-4 mt-2">
                <span className="text-sm text-slate-500">
                  <CheckCircle className="w-4 h-4 inline mr-1" />
                  Total Approved: {analytics?.approvedRequests || 0}
                </span>
                <span className="text-sm text-slate-500">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500">{analytics?.approvalRate || 0}% approval rate</p>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-red-600">{analytics?.rejectedRequests || 0}</p>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-purple-600">{analytics?.avgProcessingTime || 0}</p>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {analytics?.roomTypeStats && Object.entries(analytics.roomTypeStats).map(([type, count]) => (
            <div key={type} className="text-center p-4 bg-gradient-to-br from-slate-50 to-blue-50 rounded-xl border border-slate-200">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex flex-col md:flex-row gap-4 flex-1">
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
            <div className="relative">
              <CalendarIcon className="w-5 h-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400" />
          ...
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search approved allocations..."
              value={searchTerm}
      ...
      {/* Approved Allocations List */}
      <div className="bg-white rounded-2xl shadow-lg border border-slate-200">
        <div className="p-6 border-b border-slate-200">
          <h3 className="text-lg font-semibold text-slate-800">Approved Room Allocations</h3>
          <p className="text-sm text-slate-600 mt-1">All successfully approved room allocation requests</p>
        </div>
        ...
        <div className="p-6">
          {filteredRequests.length > 0 ? (
                    ...
                    <div className="flex-1">
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-2 mt-1">
                            ...
                            <span className="text-sm text-slate-500">
                              ...
                              Approved on {formatDate(request.processed_at)}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div className="bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-blue-200">
                          ...
                          <div className="text-xs text-slate-500">
                            {request.user_profiles?.admission_number || 'N/A'}
                          </div>
                          <div className="text-xs text-slate-500">
                            {request.user_profiles?.email || 'N/A'}
                          </div>
                        </div>
                        ...
                        <div className="bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-blue-200">
                          ...
                          {request.room_allocations?.[0]?.rooms ? (
                            <div>
                              <div className="text-sm font-semibold text-slate-800">
                                Room {request.room_allocations[0].rooms.room_number} (Floor {request.room_allocations[0].rooms.floor})
                              </div>
                              <div className="text-xs text-slate-500">
                                Capacity: {request.room_allocations[0].rooms.capacity} students
                              </div>
                            </div>
                          ) : (
                            <div className="text-sm text-slate-500">Room allocation pending</div>
                          )}
                        </div>
                      ...
                      {request.special_requirements && (
                        <div className="bg-white/60 backdrop-blur-sm rounded-xl p-4 mb-4 border border-amber-200">
                          ...
                          <p className="text-slate-600 leading-relaxed">{request.special_requirements}</p>
                        </div>
                      )}
                      ...
                      {request.notes && (
                        <div className="bg-white/60 backdrop-blur-sm rounded-xl p-4 mb-4 border border-slate-200">
                          ...
                          <p className="text-slate-600 leading-relaxed">{request.notes}</p>
                        </div>
                      )}

                      {/* Allocation Status */}
                      {request.room_allocations?.[0] && (
                        <div className="bg-white/60 backdrop-blur-sm rounded-xl p-4 mb-4 border border-green-200">
                          <div className="text-sm font-semibold text-slate-700 mb-2 flex items-center">
                          ...
                          <div className="flex items-center space-x-4">
                            <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                              request.room_allocations[0].allocation_status === 'confirmed' 
                                ? 'bg-green-100 text-green-800' 
                                : 'bg-yellow-100 text-yellow-800'
                            }`}>
                              {request.room_allocations[0].allocation_status}
                            </span>
                            <span className="text-xs text-slate-500">
                              Allocated: {formatDate(request.room_allocations[0].allocated_at)}
                            </span>
                          </div>
                        </div>
                      )}
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-lg font-semibold text-slate-800 mb-2">No Approved Allocations Found</h3>
              <p className="text-slate-600">No approved room allocations match your current filters.</p>
            </div>
          )}
...
export default AdminRoomAnalytics;
