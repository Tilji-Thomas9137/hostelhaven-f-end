...
import { 
  ...
  User, Mail, Phone, Calendar, MapPin, GraduationCap, Shield
} from 'lucide-react';
...
const StudentManagement = ({ data = [], onRefresh }) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    const studentId = selectedStudent.admission_number || selectedStudent.id;
  ...
  const handleActivate = async (studentId) => {
    if (!studentId || studentId.includes('mock') || studentId.includes('student-')) {
      showNotification('Cannot activate mock or test data', 'error');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3002/api/admission-registry/students/${studentId}/activate`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
          'Content-Type': 'application/json',
      ...
      showNotification(error.message || 'Failed to activate student', 'error');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center space-x-2">
                          ...
                          {student.status === 'approved' && (
                            <button
                              onClick={() => handleActivate(student.admission_number || student.id)}
      ...
      {showViewModal && selectedStudent && (
        <div className="fixed inset-0 z-50 pointer-events-none">
          <div className="absolute right-0 top-0 h-full w-full sm:w-[640px] bg-white shadow-2xl border-l border-slate-200 animate-slide-in-right overflow-y-auto pointer-events-auto">
            <div className="p-6 flex items-center justify-between border-b border-slate-200 animate-fade-in">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 stagger-up">
                <div className="flex items-center space-x-3">
                  <User className="w-5 h-5 text-slate-400" />
                  <div>
                    <p className="text-sm font-medium text-slate-600">Full Name</p>
                    <p className="text-slate-900">{selectedStudent.student_name || selectedStudent.full_name || '—'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-600">Admission Number</p>
                    <p className="text-slate-900">{selectedStudent.admission_number}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-900">{selectedStudent.student_email || selectedStudent.email || '—'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-900">{selectedStudent.student_phone || selectedStudent.phone || '—'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-900">{selectedStudent.course || '—'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-900">{selectedStudent.batch_year || selectedStudent.year || '—'}</p>
                  </div>
                </div>
      ...
      {showDeleteModal && selectedStudent && (
        ...
        >
          <div className="bg-white rounded-2xl p-6 w-full max-w-md relative shadow-2xl mx-4">
            {/* Close Button */}
            <button
              onClick={() => {
                setShowDeleteModal(false);
            ...
            <div className="text-center">
              ...
              <p className="text-slate-600 mb-6">
                Are you sure you want to delete <span className="font-semibold text-slate-800">{selectedStudent.student_name}</span>? 
